...
The Roles SDK is designed in three layers of abstraction:
...
Rather than a list of targets with nested functions as exposed by the subgraph, permissions are defined by a flat array of entries that grant call rights to an address or a specific function at an address.
...
A kit is a typed SDK tailored to a specific set of contracts that allows defining permissions on them in a convenient syntax that's fully supported by the type system.

Users simply provide the addresses of contracts. Powered by [eth-sdk](https://github.com/dethcrypto/eth-sdk) and [TypeChain](https://github.com/dethcrypto/TypeChain), ABIs will be automatically downloaded from Etherscan to generate type-safe functions for granting permissions.
